...
-Decorador de Log: Deve ser aplicado a todas as funções de transações(depósito, saque,criação de conta,etc); Esse decorador deve registrar(printar) a data e a hora de cada transação, bem como o tipo de transação;
-Gerador de Relatórios: deve permitir iterar sobre as transações de uma conta e rotornar, uma a uma, as transações que foram realizadas; Também deve ter uma forma de filtrar as transações baseado em seu tipo(por exemplo, apenas saques ou apenas depósitos);
-Iterador Personalizado: Implemente um Iterador Personalizado ContaIterador que permita iterar sobre todas as contas do banco, retornando informações básicas de cada conta(número da conta, saldo atual, etc.)
'''
...
class Historico:
    ...
    # Métodos:
        ...
        # (gerar_relatorio): Permite filtrar transações por tipo.
