...
'''
Objetivo: 
-Modificar o atual decorador de log, que imprime informações no console, para que ele salve essas informações em um arquivo de log, possibilitando uma revisão mais fácil e uma análise mais detalhada das operações dos usuarios;
...
def log_transacao(func):
    def envelope(*args,**kwargs):
        ...
        log_msg = f"[{now}] Função '{func.__name__}' executada com argumentos {args} e {kwargs}."
        
        #Salvando no arquivo de log
        with open('log.txt','a') as arquivo_log:
            arquivo_log.write(log_msg + '\n')
        #executando a função original e retornando o resultado
